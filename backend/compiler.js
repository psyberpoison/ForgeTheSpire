...
// cards take. `card.description` is expected to already be non-empty by
// export time (frontend/index.html's generateCardDescription() auto-fills
// it at save time whenever left blank) — the fallback string below only
// guards a package edited/imported outside the normal editor flow.
function generateCardLocalization(card) {
  const title = (card.name || 'Untitled').replace(/"/g, '\\"');
  const description = (card.description || 'See card effects.').replace(/"/g, '\\"').replace(/\n/g, '\\n');
  return `
...
function generateMechanicSource(mechanic, namespace, refMaps) {
